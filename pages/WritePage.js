...
function renderSelectedChampions() {
  ...
  container.innerHTML = "";
  const sorted = [...selectedChampions].sort((a, b) => a.cost !== b.cost ? a.cost - b.cost : a.name.localeCompare(b.name, "ko"));
...
function renderSynergyBar() {
  ...
  const tierPriority = { unique: 0, chromatic: 1, gold: 2, silver: 3, bronze: 4, darken: 5 };
  const traitArray = Object.entries(traitCount).map(([trait, count]) => {
    const info = traitsData[trait];
    if (!info) return null;
    const idx = info.thresholds.findLastIndex((t) => count >= t);
    const color = idx === -1 ? "darken" : info.colors[idx];
    return { name: trait, count, tier: color, tierOrder: tierPriority[color], icon: info.icon, thresholds: info.thresholds, activeTierIndex: idx };
  }).filter(Boolean).sort((a, b) => a.tierOrder !== b.tierOrder ? a.tierOrder - b.tierOrder : b.count - a.count || a.name.localeCompare(b.name, "ko"));
